...
|                               to be scrollable                              |
|-----------------------------------------------------------------------------|
| Created 2002-05-28 | All changes are in the log above. | Updated 2003-09-23 |
\----------------------------------------------------------------------------*/
...
Menu.prototype.show = function ( left, top, w, h ) {
	...
	// common around a bug that sometimes occured with large pages when
...
Menu.prototype.addEventHandler = function(oTarget, sEventType, fnHandler)
{
...
Menu.prototype.removeEventHandler = function(oTarget, sEventType, fnHandler) 
{
...
Menu.prototype.detachEvents = function () {
	...
	Menu.prototype.removeEventHandler( scrollContainer, "scroll", this.eventListeners.onscroll );
	Menu.prototype.removeEventHandler( d, "mouseover", this.eventListeners.onmouseover );
	Menu.prototype.removeEventHandler( d, "mouseout", this.eventListeners.onmouseout );
	Menu.prototype.removeEventHandler( d, "mouseup", this.eventListeners.onmouseup );
	Menu.prototype.removeEventHandler( d, "mousewheel", this.eventListeners.onmousewheel );
	if (this.cssText == null) {
		var linkEl = d.getElementsByTagName("LINK")[0];
		Menu.prototype.removeEventHandler( linkEl, "readystatechange", this.eventListeners.onreadystatechange );
	}
	Menu.prototype.removeEventHandler( d, "keydown", this.eventListeners.onkeydown );
	Menu.prototype.removeEventHandler( d, "contextmenu", this.eventListeners.oncontextmenu );
	// prevent IE to keep menu open when navigating away
	Menu.prototype.removeEventHandler( window, "unload", this.eventListeners.onunload );
}
...
MenuItem.prototype.getIconHtml = function () {
	return this.icon != "" ? "<img width=\"16\" src=\"" + this.icon + "\">" : "<span>&nbsp;</span>";
};
...
MenuItem.prototype.positionSubMenu = function () {
	...
	// common around for buggy graphics drivers that screw up the screen.left
...
MenuBar.prototype.toHtml = function () {
	...
	//alert("<div class=\"menu-bar\" id=\"" + this.id + "\">" +itemsHtml.join( "" ) +"</div>";);
...
MenuBar.prototype.addEventHandler = function(oTarget, sEventType, fnHandler)
{
    if (oTarget.addEventListener) {
        oTarget.addEventListener(sEventType, fnHandler, false);
    } else if (oTarget.attachEvent) {
...
MenuBar.prototype.removeEventHandler = function(oTarget, sEventType, fnHandler) 
{
    if (oTarget.removeEventListener) {
        oTarget.removeEventListener(sEventType, fnHandler, false);
    } else if (oTarget.detachEvent) {
...
MenuBar.prototype.detachEvents = function () {
	...
	MenuBar.prototype.addEventHandler( this._htmlElement, "mouseover",	this.eventListeners.onmouseover );
	MenuBar.prototype.addEventHandler( this._htmlElement, "mouseout", this.eventListeners.onmouseout );
	MenuBar.prototype.addEventHandler( this._htmlElement, "mousedown", this.eventListeners.onmousedown );
	MenuBar.prototype.addEventHandler( this._document, "keydown", this.eventListeners.onkeydown );
	MenuBar.prototype.removeEventHandler( window, "unload", this.eventListeners.onunload );
}
...
MenuBar.prototype.hookupMenu = function ( element ) {
	...
	// hook up events
	MenuBar.prototype.addEventHandler( element, "mouseover",	this.eventListeners.onmouseover );
	MenuBar.prototype.addEventHandler( element, "mouseout", this.eventListeners.onmouseout );
	MenuBar.prototype.addEventHandler( element, "mousedown", this.eventListeners.onmousedown );
	MenuBar.prototype.addEventHandler( this._document, "keydown", this.eventListeners.onkeydown );
	MenuBar.prototype.addEventHandler( window, "unload", this.eventListeners.onunload );
};
...
//
function MenuButton( sLabelText, oSubMenu, fAction,  sIconSrc) {
	this.MenuItem = MenuItem;
	...
	//}
	//else {
		this.MenuItem( sLabelText, fAction, sIconSrc, oSubMenu );
	//}
	//*****Add End*****/
...
MenuButton.prototype.toHtml = function () {
	...
	return	"<span unselectable=\"on\" " +
			...
			"<span unselectable=\"on\" class=\"middle\" "+this.getActionJS()+">" +
				"<table border=0 ><tr><td>"+this.getIconImg()+"</td><td valign=\"bottom\" style=\"font: Menu\" nowrap=\"nowrap\">"+this.getTextHtml()+"</td></tr></table>"+
			"</span>" +
	...
	return	"<span unselectable=\"on\" " +
			...
			"><span unselectable=\"on\" class=\"left\"></span>" +
			"<span unselectable=\"on\" class=\"middle\" "+this.getActionJS()+">" +
				"<table border=0 class=\"middle\"><tr><td>"+this.getIconImg()+"</td><td>"+this.getTextHtml()+"</td></tr></table>"+
			"</span>" +
			"<span unselectable=\"on\" class=\"right\"></span>" +
			"</span>";
	*/
...
//*****Add Begin*****/
MenuButton.prototype.getActionJS = function () {
	if(this.action == null || this.action == "") {
	  return "";
		...
		if(this.action.indexOf("javascript:") != -1) {
			...
			return " onclick=\"location.href='"+this.action+"'\"";	
		}
	}
	return (this.action == null || this.action == "")?"":" onclick=\""+this.action+"\"";
};
MenuButton.prototype.getIconImg = function () {
	//return "<img src=\"/commons/menu/save.gif\" width=\"16\" height=\"15\" border=0>";
	return (this.icon == null || this.icon == "")?"":"<img src=\""+this.icon+"\" border=0>";	
};
//*****Add End*****/
...
var eventListeners = {
	menu: {
		onkeydown:	function ( id ) {
			...
			var w = eventListeners.menu.onkeydown.caller.arguments[0] || oThis.getDocument().parentWindow.event;
			oThis.handleKeyEvent( w );
		},
		...
		oncontextmenu:	function ( id ) {
			var oThis = menuCache[id];
			var w = eventListeners.menu.oncontextmenu.caller.arguments[0] || oThis.getDocument().parentWindow.event;
			w.returnValue = false;
		},
		...
		onmouseover:	function ( id ) {
			...
			if(window.event){
				var w =  oThis.getDocument().parentWindow.event;
				var fromEl = getMenuItemElement( e.fromElement );
				var toEl = getMenuItemElement( e.toElement );
			}else{
				var w = eventListeners.menu.onmouseover.caller.arguments[0];
				var fromEl = getMenuItemElement( e.relatedTarget );
				var toEl = getMenuItemElement( e.target );
			}
		...
		onmouseout:	function ( id ) {
			...
			if(window.event){
				...
				var w = eventListeners.menu.onmouseout.caller.arguments[0];
				var fromEl = getMenuItemElement( e.target );
				var toEl = getMenuItemElement( e.relatedTarget );
			}
		...
		onmouseup:	function ( id ) {
			var oThis = menuCache[id];
			var w = eventListeners.menu.onmouseup.caller.arguments[0] || oThis.getDocument().parentWindow.event;

			var srcEl	= getMenuItemElement( w.target || w.srcElement );
		...
		onmousewheel:	function ( id ) {
			...
			var w = eventListeners.menu.onmousewheel.caller.arguments[0] || d.parentWindow.event;
			var scrollContainer = d.getElementById("scroll-container");
			scrollContainer.scrollTop -= 3 * w.wheelDelta / 120 * ScrollButton.scrollAmount;
		},
	...
	menuBar:	{
		onmouseover:	function ( id ) {
			...
			if(window.event){
				...
				var e = eventListeners.menuBar.onmouseover.caller.arguments[0];
				var fromEl = getMenuItemElement( e.relatedTarget );
				var toEl = getMenuItemElement( e.target );
			}
		...
		onmouseout:	function ( id ) {
			...
			if(window.event){
				...
				var e = eventListeners.menuBar.onmouseout.caller.arguments[0];
				var fromEl = getMenuItemElement( e.target );
				var toEl = getMenuItemElement( e.relatedTarget );
			}
		...
		onmousedown:	function ( id ) {
			...
			var e = eventListeners.menuBar.onmousedown.caller.arguments[0] || oThis.getDocument().parentWindow.event;
			if ( e.button != MenuBar.leftMouseButton )
				return;

			var el = getMenuItemElement( e.target || e.srcElement );
		...
		onkeydown:	function ( id ) {
			var oThis = menuCache[id];
			var e = eventListeners.menuBar.onkeydown.caller.arguments[0] || oThis.getDocument().parentWindow.event;
